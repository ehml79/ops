...
function install_php(){
    ...
    if [ -f /usr/bin/apt ];then
        echo 'ubuntu'
        sudo apt -y install git libpcre3 libpcre3-dev  zlib1g-dev build-essential libxml2-dev openssl libssl-dev make curl libcurl4-openssl-dev libjpeg-dev libpng-dev  libmcrypt-dev libcurl4-gnutls-dev libxslt-dev pkg-config libxml2-dev openssl  libfreetype6-dev  libmcrypt-dev  libsodium-dev argon2 libargon2-0 libargon2-0-dev libxml2-dev m4 autoconf libzip-dev
    ...
    cp /data/service/src/php-7.3.11/sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm

    chmod +x /etc/init.d/php-fpm
    cp /data/service/src/php-7.3.11/php.ini-production /data/service/php/etc/php.ini
    ...
    sed -i "s@user =.*@user = ${web_user}@" /data/service/php/etc/php-fpm.d/www.conf
    sed -i "s@group =.*@group = ${web_user}@" /data/service/php/etc/php-fpm.d/www.conf

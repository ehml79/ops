...
mongodb_oplog=/data/backup/mongodb/mongodb_oplog

 
if [ ! -d "${mongodb_oplog}/mongo-${mongodb_port}" ];then
    mkdir -p ${mongodb_oplog}/mongo-${mongodb_port}
fi
 
if [ ! -d "${mongodb_oplog}/log-${mongodb_port}" ];then
    mkdir -p ${mongodb_oplog}/log-${mongodb_port}
fi
 
bkdatapath=${mongodb_oplog}/mongo-${mongodb_port}
bklogpath=${mongodb_oplog}/log-${mongodb_port}
...
opmes=$(/bin/echo "db.printReplicationInfo()" | /data/service/mongodb/bin/mongo  ${mongodb_host}:${mongodb_port} --quiet)
 
echo $opmes > /tmp/opdoctime${mongodb_port}.tmplog
opbktmplogfile=/tmp/opdoctime${mongodb_port}.tmplog
...
mongodump -h ${mongodb_host} --port ${mongodb_port}  -d local -c oplog.rs  --query '{ts:{$gte:Timestamp('$paramBakStartDate',1),$lte:Timestamp('$paramBakEndDate',9999)}}' -o $bkdatapath/mongodboplog$bkfilename
